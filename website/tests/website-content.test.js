...
test('conversion and contact paths stay intact', () => {
  ...
  assert.ok(advertise.includes('$99/mo'), 'advertise page should show Prime Placement pricing');
  assert.ok(advertise.includes('$39/mo'), 'advertise page should show Inline Listing pricing');
  assert.ok(advertise.includes('From $12'), 'advertise page should show Event Spotlight pricing');
...
test('global CSS includes responsive and brand-critical styles', () => {
  ...
  for (const token of ['--teal', '--teal-dark', '--launcher-teal', '--brand-blue', '--amber', '.hero', '.howitworks', '.photo-strip', '.home-cta-band', '.packages-grid', '.footer', '.legal-markdown']) {
