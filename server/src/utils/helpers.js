...
function transformPlayground(p) {
    ...
    const keyRaw = process.env.GOOGLE_MAPS_API_KEY;
    const mapsKey = keyRaw && String(keyRaw).trim() && String(keyRaw).trim() !== 'undefined'
        ? String(keyRaw).trim()
        : '';
    const flattened = { ...p };
    ...
    if (p.imageUrls) {
        flattened.imageUrls = p.imageUrls.map((url) => {
            ...
            if (url.startsWith('google_photo:')) {
                ...
                return `https://maps.googleapis.com/maps/api/place/photo?maxwidth=1600&photoreference=${photoRef}&key=${mapsKey}`;
            }
    ...
    // Drinking-water amenity renamed: bottle filler / refill station (not legacy public fountain flag).
    const rawBottle = flattened.hasBottleFiller;
    const rawLegacyFountain = flattened.hasWaterFountain;
    delete flattened.hasWaterFountain;
    if (rawBottle === true || rawLegacyFountain === true) {
        flattened.hasBottleFiller = true;
    } else if (rawBottle === false && rawLegacyFountain === false) {
        flattened.hasBottleFiller = false;
    } else {
        flattened.hasBottleFiller = null;
    }
