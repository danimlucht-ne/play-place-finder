...
function validate(aggregatedDetections, playground, photoCount = 0) {
  ...
  const recordAmenities = Object.entries(amenityMap)
    .filter(([, field]) => {
      if (playground[field] === true) return true;
      if (field === 'hasBottleFiller' && playground.hasWaterFountain === true) return true;
      return false;
    })
...
async function mergeAndRevalidate(newDetections, playgroundId) {
  ...
  for (const amenity of (newDetections.amenities || [])) {
    ...
    const alreadyTrue = field && (
      playground[field] === true
      || (field === 'hasBottleFiller' && playground.hasWaterFountain === true)
    );
    if (field && !alreadyTrue) {
