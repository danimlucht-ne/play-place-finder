...
function isBlockedPlaceholderEmail(email) {
...
async function sendAdminNotificationEmail(subject, textContent, htmlContent) {
    ...
    if (isBlockedPlaceholderEmail(ADMIN_EMAIL)) {
        console.warn(`Skipped admin notification email to placeholder address: ${ADMIN_EMAIL}`);
        return;
    }
...
async function sendEmail(to, subject, textContent, htmlContent) {
    ...
    if (isBlockedPlaceholderEmail(to)) {
        console.warn(`Skipped email to placeholder address: ${to}`);
        return;
    }
    try {
...
module.exports = { sendAdminNotificationEmail, sendEmail, isBlockedPlaceholderEmail };
