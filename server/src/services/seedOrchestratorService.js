 ...
 */
const SEED_ALGORITHM_VERSION = 3;
...
async function runLightweightAlgorithmRecrawl(regionKey, lat, lng) {
    ...
    const gridPoints = generateSearchGrid(lat, lng);
    try {
        const allRaw = [];
        ...
        console.log(
            `[seed] Algorithm v${SEED_ALGORITHM_VERSION} lightweight recrawl ${regionKey}: ${inserted} inserted, ${campusDiscovery.candidatesInserted} campus subvenues inserted, ${newPlaces.length} kid-filtered candidates`,
        );
...
async function handleHybridSearch(lat, lng, userId) {
    ...
    enqueueBackgroundExpansion(regionKey, lat, lng);
...
/**
 * Generates a 3×3 grid of lat/lng points centered on (lat, lng),
 * spaced ~10 miles (~16,093m) apart. Used to maximize Places API coverage.
 */
function generateSearchGrid(lat, lng) {
    const STEP_DEG_LAT = 10 / 69.0;                          // ~10 miles in degrees latitude
    const STEP_DEG_LNG = 10 / (69.0 * Math.cos(lat * Math.PI / 180)); // adjust for longitude
    const offsets = [-1, 0, 1];
    const points = [];
    ...
    return points; // 9 points
}

/**
 * Runs the full seeding process in the background.
 */
async function enqueueBackgroundExpansion(regionKey, lat, lng) {
    const db = getDb();
    try {
        const SEARCHES = backgroundExpansionSearches();

        // 3×3 grid search — each point covers ~15-mile radius, overlapping for full coverage
        const gridPoints = generateSearchGrid(lat, lng);
        console.log(`[seed] Running 3×3 grid expansion for ${regionKey} (${gridPoints.length} points × ${SEARCHES.length} searches)`);
