...
async function getAd(cityId, placement) {
  ...
  const slotKey = placement === 'featured_home' ? 'featured' : 'sponsored';
  const slotsRemaining = phaseInfo.slotsRemaining[slotKey] ?? 0;

  if (phaseInfo.phase === 'seeding') {
    return { ad: houseAd(cityId, placement, 'Advertise Your Business'), type: 'house', cityPhase: 'seeding', slotsRemaining: 0 };
  }
  ...
  if (citySettings && citySettings.houseAd) {
    return { ad: { id: `house_${cityId}`, campaignId: null, ...citySettings.houseAd, ctaText, placement }, type: 'house', cityPhase: phaseInfo.phase, slotsRemaining };
  }
  return { ad: houseAd(cityId, placement, ctaText), type: 'house', cityPhase: phaseInfo.phase, slotsRemaining };
}
...
async function getAllAds(cityId, placement) {
  ...
  if (phaseInfo.phase === 'seeding') {
    return { ads: [houseAd(cityId, placement, 'Advertise Your Business')], type: 'house', cityPhase: 'seeding', slotsRemaining: 0 };
  }
  ...
  if (campaigns.length > 0) {
    ...
    if (ads.length > 0) {
      return { ads, type: 'paid', cityPhase: phaseInfo.phase, slotsRemaining };
    }
    // Matched campaigns but nothing survived dedupe / payload shaping — fall back so clients never get paid+[].
  }

  const ctaText = 'Advertise Your Business Here';
  return { ads: [houseAd(cityId, placement, ctaText)], type: 'house', cityPhase: phaseInfo.phase, slotsRemaining };
}
