...
jest.mock('../services/cityPhaseService', () => ({
  ...
  evaluateAllCityTransitions: jest.fn(),
}));
...
describe('campaignLifecycleService', () => {
  beforeEach(() => {
    ...
    cityPhaseService.getCityPhase.mockResolvedValue({ phase: 'launch' });
  ...
  test('activateCampaign reuses a pending-review campaign and promotes it to active or scheduled', async () => {
    ...
    await expect(activateCampaign('sub-1')).resolves.toEqual({ campaignId });

    expect(updateOne).toHaveBeenCalledWith(
      { _id: campaignId },
      { $set: { status: 'active', updatedAt: new Date('2026-04-09T12:00:00Z') } },
    );
    expect(notifyCampaignLifecycleAfterActivation).toHaveBeenCalledWith('sub-1');
  });
  ...
  test('activateCampaign creates event campaigns with targeting rows and slot decrements', async () => {
    ...
    expect(cityPhaseService.decrementSlot).toHaveBeenCalledWith('omaha-ne', 'sponsored');
    expect(cityPhaseService.decrementSlot).toHaveBeenCalledWith('bellevue-ne', 'sponsored');
  ...
  test('processLifecycleTransitions captures due submissions and creates campaigns after payment succeeds', async () => {
    ...
    expect(cityPhaseService.evaluateAllCityTransitions).toHaveBeenCalled();
  });
  ...
  test('processLifecycleTransitions activates scheduled campaigns, completes ended campaigns, expires events, and releases targeting', async () => {
    ...
    expect(cityPhaseService.incrementSlot).toHaveBeenCalledWith('papillion-ne', 'featured');
