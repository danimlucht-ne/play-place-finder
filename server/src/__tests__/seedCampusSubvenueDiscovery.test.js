...
describe('generateSearchGrid', () => {
  test('returns 3×3 grid (9 points) for small metro with no viewport', () => {
    const points = generateSearchGrid(41.25, -96.01);
    expect(points).toHaveLength(9);
  });

  test('returns 3×3 grid for small viewport (Omaha-sized)', () => {
    const viewport = {
  ...
  test('returns 3×3 grid for medium viewport (extra args ignored; fixed ~10mi step grid)', () => {
    const viewport = {
  ...
  test('returns 3×3 grid for large viewport (extra args ignored)', () => {
    const viewport = {
  ...
  test('grid center point matches input coordinates', () => {
    const points = generateSearchGrid(41.25, -96.01);
  ...
  test('respects SEED_MAX_GRID_SIZE env cap', () => {
    process.env.SEED_MAX_GRID_SIZE = '3';
    const viewport = {
      northeast: { lat: 42.2, lng: -87.2 },
      southwest: { lat: 41.3, lng: -88.1 },
    };
    const points = generateSearchGrid(41.75, -87.65, { viewport });
    expect(points).toHaveLength(9); // capped to 3×3
    delete process.env.SEED_MAX_GRID_SIZE;
  });
});
