...
jest.mock('../services/cityPhaseService', () => ({
  PHASE_SLOT_LIMITS: { growth: { featured: 1, sponsored: 2 }, mature: { featured: 1, sponsored: 8 } },
...
describe('adSubmissionRoutes', () => {
  ...
  test('creates first advertiser submission and bootstraps seeding city to beta', async () => {
    const advertiserId = new ObjectId();
    const submissionId = new ObjectId();
    ...
    getDb.mockReturnValue(makeDb({
      ...
      cityAdSettings: { updateOne: updateCity },
      adSubmissions: { insertOne: insertSubmission },
    }));
    cityPhaseService.getCityPhase.mockResolvedValue({ phase: 'seeding' });
    ...
    expect(updateCity).toHaveBeenCalledWith(
      { cityId: 'omaha-ne' },
      expect.objectContaining({ $set: expect.objectContaining({ phase: 'growth', phaseOverride: true }) }),
      { upsert: true },
    );
  ...
  test('saves package selection with server-side pricing and radius reach checks', async () => {
    ...
    cityPhaseService.checkSlotAvailability.mockResolvedValue({ available: true, remaining: 1 });
