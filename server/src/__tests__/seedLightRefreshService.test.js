...
describe('seedLightRefreshService', () => {
  ...
  test('lightRefreshBudget includes maxAnchorPoints (prevents unbounded anchor query)', () => {
    const b = lightRefreshBudget({
      maxAnchorPoints: 42,
      maxNearbyCalls: 2,
      radiusMeters: 1000,
      maxDetailsCalls: 5,
    });
    expect(b.maxAnchorPoints).toBe(42);
    expect(b).toEqual(
