...
jest.mock('../services/cityPhaseService', () => ({
  ...
  PHASE_SLOT_LIMITS: {
    growth: { featured: 2, sponsored: 10 },
...
describe('adAdminRoutes', () => {
  ...
  test('city, region, advertiser, and lifecycle admin utility routes call the correct services', async () => {
    ...
    getDb.mockReturnValue(makeDb({
      cityAdSettings: {
        find: jest.fn().mockReturnValue(cursor([{ cityId: 'omaha-ne', phase: 'growth' }])),
    ...
    await request(buildApp()).put('/cities/omaha-ne/phase').send({ phase: 'growth' }).expect(200);
    expect(cityPhaseService.setPhaseOverride).toHaveBeenCalledWith('omaha-ne', 'growth');

    await request(buildApp()).post('/cities/omaha-ne/open-advertising').expect(200);
    expect(updateCity).toHaveBeenCalledWith(
      { cityId: 'omaha-ne' },
      expect.objectContaining({ $set: expect.objectContaining({ phasePricing: expect.any(Object) }) }),
      { upsert: true },
    );
