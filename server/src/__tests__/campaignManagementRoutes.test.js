...
describe('campaignManagementRoutes', () => {
  ...
  test('moves scheduled creative edits back to pending review and creates a review flag', async () => {
    ...
    campaignEditService.updateCreative.mockResolvedValue({ success: true, reviewRequired: true });
    ...
    expect(insertReviewFlag).toHaveBeenCalledWith(expect.objectContaining({
      ...
      description: 'User changed campaign details before launch. Review is required before the ad can run.',
    }));
    expect(res.body).toEqual({ message: 'success' });
  });

  test('active campaign creative edits are queued for manual review without taking the campaign offline', async () => {
    const campaignId = new ObjectId();
    const advertiserId = new ObjectId();
    const submissionId = new ObjectId();
    const updateCampaign = jest.fn();
    ...
    expect(insertReviewFlag).toHaveBeenCalledWith(expect.objectContaining({
      submissionId,
      flagType: 'user_requested_changes',
      description: 'User changed a live campaign. Keep the current ad running and review the update before it replaces the approved version.',
      createdBy: 'user-1',
    }));
    expect(res.body).toEqual({ message: 'success_pending_review' });
  });
