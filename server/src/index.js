    ...
    if (process.env.NODE_ENV !== 'test') {
        ...
        setInterval(async () => {
            try {
                ...
                const total = (transitions.activated || 0) + (transitions.completed || 0) + (transitions.eventExpired || 0) + (expirations.expired || 0) + (expiringSent || 0) + (midLoyaltyIssued || 0);
                if (total > 0 && process.env.LOG_LIFECYCLE_CRON === '1') {
                    console.log(`[lifecycle-cron] activated=${transitions.activated}, completed=${transitions.completed}, eventExpired=${transitions.eventExpired || 0}, introExpired=${expirations.expired}, expiringNotified=${expiringSent}, midLoyaltyIssued=${midLoyaltyIssued || 0}`);
                }
            } catch (err) {
