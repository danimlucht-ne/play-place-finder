...
// GET /cities — list all cities with phase info and slot status
router.get('/cities', async (req, res) => {
  try {
    ...
    const data = cities.map(c => ({
      cityId: c.cityId,
      phase: c.phase === 'beta' ? 'growth' : (c.phase || 'seeding'),
      slots: c.slots || { featured: { max: 0, remaining: 0 }, sponsored: { max: 0, remaining: 0 } },
...
router.put('/cities/:cityId/phase', async (req, res) => {
  try {
    ...
    if (!phase || !['seeding', 'growth', 'mature'].includes(phase)) {
      return res.status(400).json({ error: 'phase must be one of: seeding, growth, mature' });
    }
...
// POST /cities/:cityId/open-advertising — transition seeding city to growth (slots + pricing)
router.post('/cities/:cityId/open-advertising', async (req, res) => {
  try {
    ...
    if (currentPhase === 'growth' || currentPhase === 'mature') {
      return res.status(400).json({ error: `City is already in ${currentPhase} phase` });
    }

    await cityPhaseService.setPhaseOverride(cityId, 'growth');

    const growthSlots = cityPhaseService.PHASE_SLOT_LIMITS.growth;
    const now = new Date();
    ...
    await db.collection('cityAdSettings').updateOne(
      { cityId },
      {
        $set: {
          slots: {
            featured: { max: growthSlots.featured, remaining: growthSlots.featured },
            sponsored: { max: growthSlots.sponsored, remaining: growthSlots.sponsored },
          },
          phasePricing: {
    ...
    res.json({ message: 'success', data: { cityId, phase: 'growth' } });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
