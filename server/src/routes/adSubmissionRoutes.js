    ...
    if (previousSubmission.creativeId) {
      ...
      if (originalCreative) {
        const creativeCopy = {
          ...
          imageAlignment: originalCreative.imageAlignment || 'center',
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!cityPhase.advertisingOpen) {
      const existingAdvertiserCount = await db.collection('advertisers').countDocuments({ regionKey });

      if (existingAdvertiserCount === 0) {
        await cityPhaseService.openAdvertisingForRegion(regionKey, {
          phasePricing: {
            growing: { featured: 25000, sponsored: 9900, event_7d: 2500, event_14d: 5000 },
            mature: { featured: 29900, sponsored: 12900, event_7d: 3300, event_14d: 6500 },
          },
        });
      } else {
  ...
  try {
    ...
    if (numericStep === 2) {
      ...
      const allowedPackages = [
        ...
        'event_spotlight_7d',
        'event_spotlight_14d',
        'event_spotlight_7d_home',
        'event_spotlight_14d_home',
      ];
      if (!packageType || !allowedPackages.includes(packageType)) {
        console.warn(`[put-submission] step2 reject: bad packageType (${packageType})`);
        return res.status(400).json({
          error:
            'packageType must be featured_home, inline_listing, event_spotlight_7d, event_spotlight_14d, event_spotlight_7d_home, or event_spotlight_14d_home',
        });
      }
      ...
      if (isEventPackage) {
        ...
        const durationDays = String(packageType).includes('14d') ? 14 : 7;
    ...
    if (numericStep === 3) {
      let { headline, body, ctaText, ctaUrl, imageUrl, additionalImageUrls, imageAlignment } = req.body;
      ...
      const alignRaw = imageAlignment != null ? String(imageAlignment).trim().toLowerCase() : '';
      const imageAlignmentNorm = ['top', 'center', 'bottom'].includes(alignRaw) ? alignRaw : 'center';
      ...
      const creativePayload = {
        ...
        imageAlignment: imageAlignmentNorm,
        templateType: 'standard',
        status: 'draft',
        ...eventFields,
        updatedAt: now,
