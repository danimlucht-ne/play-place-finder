  ...
  try {
    ...
    if (['scheduled', 'pending_review'].includes(campaign.status)) {
      ...
      await db.collection('reviewFlags').insertOne({
        submissionId: campaign.submissionId,
        flagType: 'user_requested_changes',
        description: 'User changed event details before launch. Re-review required.',
        severity: 'medium',
        autoGenerated: false,
        resolvedAt: null,
        createdAt: now,
...
router.post('/:id/cancel', async (req, res) => {
  try {
    ...
    // Release slots in all targeted cities
    ...
    for (const rk of regionKeys) {
      try { await cityPhaseService.incrementSlot(rk, slotType); } catch (_) {}
    }
