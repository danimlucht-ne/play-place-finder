...
function firebaseActionCodeSettings(path) {
    const baseUrl = (process.env.FIREBASE_AUTH_CONTINUE_URL || process.env.MARKETING_SITE_BASE_URL || '').trim().replace(/\/+$/, '');
...
router.post('/register', async (req, res) => {
    ...
    /** If createUser succeeded but a later step failed, roll back or the next signup hits email-already-exists. */
    let rollbackUid = null;
    try {
        ...
        if (rollbackUid) {
            try {
                await getDb().collection('users').deleteOne({ _id: rollbackUid });
            } catch (rollbackDbErr) {
                console.error('Register rollback: failed to remove users doc:', rollbackDbErr.message);
            }
            try {
                await admin.auth().deleteUser(rollbackUid);
            } catch (rollbackAuthErr) {
                console.error('Register rollback: failed to delete Firebase user:', rollbackAuthErr.message);
            }
        }
        ...
        const scopeDenied = Array.isArray(details) && details.some((d) => d && d.reason === 'ACCESS_TOKEN_SCOPE_INSUFFICIENT');
        if (scopeDenied || String(err.response?.data?.error?.message || '').includes('insufficient authentication scopes')) {
            return res.status(500).json({
                error:
                    'FIREBASE_WEB_API_KEY is rejected by Google Identity Toolkit (often: API key is Android/iOS-restricted '
                    + 'or missing the Identity Toolkit API). Use the Web API key from Firebase Console → Project settings, '
                    + 'or create a separate key with Identity Toolkit API enabled for server-side token exchange.',
            });
        }
...
module.exports.firebaseActionCodeSettings = firebaseActionCodeSettings;
