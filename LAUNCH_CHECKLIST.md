# PlayPlace Finder — project checklist (single source)

**Everything in one place:** GCP deploy, Stripe, Android/Play Console, monitoring, roadmap — plus **play-place-finder.com** trust, SEO, accessibility, and compliance. Use checkboxes in the tail section alongside the numbered phases; migrate stale lines to your issue tracker when you outgrow this file.
...
## play-place-finder.com — trust, compliance & growth

The phases above include Play Console **Data safety** and store listing work (Phase 4). The items below extend that to the **website** (`website/`), **lawyer-facing** policies, SEO, and operations — check them even when mobile launch is “done.”

### Trust, positioning & conversion

- [ ] **Homepage first screen:** who it is for (parents, caregivers; clarify if kids are not primary account holders) and what they get in one plain sentence.
- [ ] **Safety & moderation:** how listings / photos / reviews are handled if UGC exists; **report** path visible.
- [ ] **Data accuracy** disclaimer for venue/playground info (“verify hours with venue” — lawyer to tune).
- [ ] **Support:** contact method + expected response time (even approximate business days).
- [ ] **App store alignment:** listing copy, screenshots, privacy nutrition labels, age rating — match **actual** data collection (see Phase 4; keep web + app consistent).
- [ ] (Optional) **FAQ** for recurring parent questions (parking, accessibility, “how is this free?”).

### SEO & sharing

- [ ] Unique **`<title>` + meta description** per major route (home, privacy, terms, advertise, login, hubs, etc.).
- [ ] **Open Graph + Twitter Card** + **`og:image`** (1200×630) — important for Facebook / iMessage previews.
- [ ] **Local / regional SEO** if you target specific states or metros — structured data where appropriate (lawyer + SEO guidance).
- [ ] Branded **`404`** and soft **offline** messaging if the app shell can fail network.
- [ ] **Sitemap / robots** for the Next.js / static export (`website` app).

### Accessibility & inclusive UX

- [ ] **Keyboard + screen reader** pass on listing flows and **login**.
- [ ] **Color contrast** on maps + overlays + small metadata text.
- [ ] **Motion:** respect `prefers-reduced-motion` for map or list animations.
- [ ] **Forms:** clear labels, errors in plain language; **advertiser** flows (`/advertise`, advertiser hub).

### Location, children & privacy (lawyer)

- [ ] **COPPA / state privacy laws:** who may create accounts; parental consent if any under-13 contact is possible.
- [ ] **Location data:** precise vs coarse; retention; deletion; background location on mobile (if any).
- [ ] **Photos of children:** uploads policy (prohibit, moderate, EXIF stripping) — lawyer + product.
- [ ] **Third-party maps / APIs:** terms, attribution, disclosure in Privacy.
- [ ] **Analytics / crash reporting:** subprocessors; opt-out or consent if required in your jurisdictions.
- [ ] **Marketing email / SMS** (if any): consent, unsubscribe, recordkeeping.

### Advertising & monetization

- [ ] **Advertiser agreement** (`/advertiser-agreement`) reviewed with lawyer: payment terms, prohibited content, indemnity, termination, refunds.
- [ ] **Ad creative standards** (family brand): no misleading claims; clear **“Ad”** labeling if required.
- [ ] **Stripe / payouts** for advertisers: tax forms (W-9 / 1099 posture) — accountant + lawyer.
- [ ] **Children-directed advertising** restrictions — lawyer (FTC / state rules).

### Pricing, communication & operations

- [ ] **Consumer pricing** (if any): free vs paid tiers; trial language; refund policy.
- [ ] **B2B / venue** deals (if venues pay): contract template + insurance considerations.
- [ ] **Incident comms:** wrong listings or safety-related reports — who responds and SLA.
- [ ] **Data breach response** plan (users, regulators per law) — lawyer-drafted template.

### Legalities (lawyer checklist)

- [ ] **Terms of Use** (end users + any pro accounts).
- [ ] **Privacy Policy** accurate to actual data flows (website + server + mobile); children’s section if applicable.
- [ ] **DMCA / copyright** path if users upload photos or text.
- [ ] **UGC moderation** terms + repeat infringer policy if needed.
- [ ] **Limitation of liability** for venue info / accidents / third-party actions.
- [ ] **Insurance:** GL / cyber especially if you handle payments or user-generated location content at scale.

### Optional “nice next”

- [ ] **Partnership** page (libraries / parks departments) if outreach is a channel.
- [ ] **Press kit** (logo, screenshots, one-pager).
- [ ] **Security** page (high-level: encryption in transit, no selling data) — avoid security theater.
