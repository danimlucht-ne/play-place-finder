...
fun AdminRegionMaintenanceScreen(
    ...
    var staleReasonText by remember { mutableStateOf("") }
    var staleLoading by remember { mutableStateOf(false) }
    var staleResult by remember { mutableStateOf<String?>(null) }
    var staleError by remember { mutableStateOf<String?>(null) }
    var backfillLoading by remember { mutableStateOf(false) }
    var backfillResult by remember { mutableStateOf<String?>(null) }
    var backfillError by remember { mutableStateOf<String?>(null) }
    var reconcileLoading by remember { mutableStateOf(false) }
    var reconcileResult by remember { mutableStateOf<String?>(null) }
    var reconcileError by remember { mutableStateOf<String?>(null) }

    fun targetRegionKeySingleOrNull(): String? {
        val m = manualRegionKey.trim()
        if (m.isNotEmpty()) return m
        return (regionPick as? RegionPick.One)?.region?.regionKey
    }
    ...
    Column(
        ...
        Text("Staleness & seed coverage (one region)", fontWeight = FontWeight.SemiBold, fontSize = 14.sp)
        Text(
            "Mark stale when user submissions signal the map is hotter than the calendar. Backfill writes `seed_tiles` " +
                "from existing pins (additive; never deletes). Reconcile runs merge/campus pass only — set " +
                "SEED_VIEWPORT_RECONCILE_ENABLED=1 on the server.",
            fontSize = 12.sp,
            color = Color(0xFF757575),
        )
        OutlinedTextField(
            value = staleReasonText,
            onValueChange = { staleReasonText = it },
            label = { Text("Note (optional) for mark-stale") },
            modifier = Modifier.fillMaxWidth(),
            singleLine = false,
            minLines = 1,
        )
        staleError?.let { Text(it, color = FormColors.ErrorText, fontSize = 13.sp) }
        staleResult?.let { Text(it, fontSize = 12.sp, color = Color(0xFF2E7D32)) }
        backfillError?.let { Text(it, color = FormColors.ErrorText, fontSize = 13.sp) }
        backfillResult?.let { Text(it, fontSize = 12.sp, color = Color(0xFF2E7D32)) }
        reconcileError?.let { Text(it, color = FormColors.ErrorText, fontSize = 13.sp) }
        reconcileResult?.let { Text(it, fontSize = 12.sp, color = Color(0xFF2E7D32)) }
        Row(
            modifier = Modifier.fillMaxWidth(),
            ...
            OutlinedButton(
                onClick = {
                    val rk = targetRegionKeySingleOrNull()
                    if (rk.isNullOrBlank()) {
                        staleError = "Pick one region from the menu or enter a manual regionKey (not All)."
                    ...
                    scope.launch {
                        ...
                        staleResult = null
                        try {
                            val r = service.adminMarkRegionStale(rk, staleReasonText.trim().ifBlank { null })
                            val err = r["error"]?.toString()
                            if (err != null) staleError = err
                            else {
                                @Suppress("UNCHECKED_CAST")
                                val data = r["data"] as? Map<String, Any?>
                                @Suppress("UNCHECKED_CAST")
                                val lr = data?.get("lightRefresh") as? Map<String, Any?>
                                val enq = lr?.get("enqueued") as? Boolean
                                val reason = lr?.get("reason")?.toString()
                                staleResult = buildString {
                                    append("Marked $rk stale (clears on next successful light refresh). ")
                                    when (enq) {
                                        true -> append("Light refresh queued.")
                                        false -> append(
                                            if (reason != null) "Light refresh not queued: $reason."
                                            else "Light refresh not queued (may already be running).",
                                        )
                                        else -> append("See server logs for light refresh status.")
                                    }
                                }
                            }
                        } catch (e: Exception) {
                            staleError = e.message ?: "Request failed"
                        } finally {
                            staleLoading = false
                        }
                    }
                },
                enabled = !staleLoading && !backfillLoading && !reconcileLoading,
                modifier = Modifier.weight(1f),
            ) { Text("Mark region stale", fontSize = 13.sp) }
            OutlinedButton(
                onClick = {
                    val rk = targetRegionKeySingleOrNull()
                    if (rk.isNullOrBlank()) {
                        backfillError = "Pick one region or enter a manual regionKey."
                        return@OutlinedButton
                    }
                    scope.launch {
                        ...
                        backfillResult = null
                        try {
                            val r = service.adminBackfillSeedTiles(rk)
                            val err = r["error"]?.toString()
                            if (err != null) backfillError = err
                            else {
                                @Suppress("UNCHECKED_CAST")
                                val d = r["data"] as? Map<String, Any?>
                                backfillResult =
                                    d?.let { "Tiles (region total)=${it["tilesUpserted"]}, scanned=${it["playgroundsScanned"]}" }
                                        ?: r["message"]?.toString() ?: r.toString()
                            }
                        } catch (e: Exception) {
                            backfillError = e.message ?: "Request failed"
                        } finally {
                ...
                enabled = !staleLoading && !backfillLoading && !reconcileLoading,
                modifier = Modifier.weight(1f),
            ) { Text("Backfill seed tiles", fontSize = 13.sp) }
        }
        Row(modifier = Modifier.fillMaxWidth()) {
            OutlinedButton(
                onClick = {
                    val rk = targetRegionKeySingleOrNull()
                    if (rk.isNullOrBlank()) {
                        reconcileError = "Pick one region or enter a manual regionKey."
                        return@OutlinedButton
                    }
                    scope.launch {
                        ...
                        try {
                            val p = service.adminViewportReconcileNoNewPlaces(rk)
                            reconcileResult = "Reconcile scheduled (gridPointCount=${p.gridPointCount}, mode=reconcile). Check server logs if 403."
                        } catch (e: Exception) {
                            reconcileError = e.message ?: "Request failed"
                        } finally {
                ...
                enabled = !staleLoading && !backfillLoading && !reconcileLoading,
                modifier = Modifier.fillMaxWidth(),
            ) { Text("Reconcile (merge only, no new Places)", fontSize = 13.sp) }
        }
        if (staleLoading || backfillLoading || reconcileLoading) {
            LinearProgressIndicator(Modifier.fillMaxWidth())
        }
