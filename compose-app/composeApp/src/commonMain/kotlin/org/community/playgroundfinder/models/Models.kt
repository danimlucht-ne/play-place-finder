...
data class AdCreative(
    ...
    /** Crop focal when the image is taller than the frame: top | center | bottom */
...
data class AdCampaign(
    ...
    /** Staged creative on the linked submission when its id differs from the live creative on the campaign (edits pending review). */
    val pendingCreativePreview: CampaignCreativePreview? = null,
...
data class CityPhaseInfo(
    ...
    /** When true, the region has a `cityAdSettings` row and paid regional pricing applies. */
    val advertisingOpen: Boolean = false,
    @Deprecated("Slot inventory removed; server no longer sends counts")
    val slotsRemaining: SlotCounts? = null,
...
data class CityPhaseListItem(
    val cityId: String = "",
    val phase: String = "seeding",
    val phaseOverride: Boolean = false,
    @Deprecated("Slot inventory removed")
    val slots: CitySlots? = null,
