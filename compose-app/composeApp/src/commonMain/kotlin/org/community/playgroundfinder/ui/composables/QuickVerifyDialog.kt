...
fun QuickVerifyDialog(
    ...
    /** Saved profile name when [needsDisplayName] is false; used for attribution preview. */
    savedContributorDisplayName: String?,
    verifyDisplayName: String,
    onVerifyDisplayNameChange: (String) -> Unit,
    ...
    val nameFilled = isValidContributorNameFormat(verifyDisplayName)
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                if (isSuccess) {
                    ...
                    val effectiveWho = when {
                        needsDisplayName && isValidContributorNameFormat(verifyDisplayName) ->
                            normalizeContributorDisplayName(verifyDisplayName)
                        needsDisplayName -> "Anonymous"
                        else -> contributorPublicLabel(savedContributorDisplayName)
                    }
                    Text(
                        contributorAttributionLine("Verifying", effectiveWho),
                        fontSize = 12.sp,
                    ...
                    if (selectedRating != null) {
                        Text(
                            contributorAttributionLine("Rating", effectiveWho) + " ($selectedRating of 5)",
                            fontSize = 11.sp,
                            color = Color(0xFF424242),
                        )
                    } else {
                        Text(
                            "An optional star rating uses the same name.",
                            fontSize = 11.sp,
                    ...
                    if (needsDisplayName) {
                        ...
                        OutlinedTextField(
                            ...
                            onValueChange = { if (it.length <= 30) onVerifyDisplayNameChange(it) },
                            label = { Text("Display name") },
                            placeholder = { Text("e.g. Jamie or Jamie Smith") },
                            singleLine = true,
